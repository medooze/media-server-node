...
class IncomingStreamTrack
{
	constructor(media,id,receiver,sources)
	{
		...
		for (let id of Object.keys(sources))
		{
			...
			this.trackInfo.addSSRC(source.rtx.ssrc);
			source.fec && this.trackInfo.addSSRC(source.fec.ssrc);
			
			//Add RTX and FEC groups
			this.trackInfo.addSourceGroup(new SourceGroupInfo("FID",[source.media.ssrc,source.rtx.ssrc]));
			source.fec && this.trackInfo.addSourceGroup(new SourceGroupInfo("FEC-FR",[source.media.ssrc,source.fec.ssrc]));
	...
	*/
	...
	attached() 
	{
		...
		if (this.counter===1)
			...
			*
			* @event IncomingStreamTrack#attached
			* @type {object}
			*/
	...
	detached()
	{
		...
		if (this.counter===0)
			...
			* IncomingStreamTrack stopped event
			*
			* @event IncomingStreamTrack#detached
			* @type {object}
			*/
