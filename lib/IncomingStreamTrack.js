...
class IncomingStreamTrack
{
	...
	getStats()
	{
		...
		//For each source
		for (let id of Object.keys(this.encodings))
			//Push new encoding
			stats[id] = {
				media : getStatsFromIncomingSource(this.encodings[id].source.media),
				rtx   : getStatsFromIncomingSource(this.encodings[id].source.rtx),
				fec   : getStatsFromIncomingSource(this.encodings[id].source.fec)
			};
