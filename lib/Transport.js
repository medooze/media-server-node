...
class Transport extends Emitter
{
		...
		/** @type {import("./Endpoint").CreateTransportOptions} */ options)
	{
		...
		this.ondatachannelopen = (endpointIdentifier, dataChannel) => {
			
			if (!this.dataChannels.has(endpointIdentifier))
			{
				this.dataChannels.set(endpointIdentifier, new Map());
			}
			
			let dc = new DataChannel(endpointIdentifier, dataChannel);
			
			this.dataChannels.get(endpointIdentifier).set(dataChannel.GetId(), dc);
			
			this.emit("datachannelopen", dc);
		};
		
		this.ondatachannelclose = (endpointIdentifier, dataChannel) => {
			...
			let dc = this.dataChannels.get(endpointIdentifier).get(dataChannel.GetId());
			this.dataChannels.get(endpointIdentifier).delete(dc.GetId());
		...
		// Get current data channels
		let dataChannels = this.transport.GetDataChannels();
		for (let [endpoingId, dataChannel] of dataChannels)
		{
			if (!this.dataChannels.has(endpoingId))
			{
				this.dataChannels.set(endpoingId, new Map());
			}
			
			let dc = new DataChannel(endpoingId, dataChannel);
			
			this.dataChannels.get(endpoingId).set(dataChannel.GetId(), dc);
		}
	...
	/**
	 * 
	 * @returns {Map<String, Array<DataChannel>>}
	 */
	getDataChannels()
	{
	...
	getDataChannels()
	{
		return this.transport.GetDataChannels();
	}
