...
class Transport extends Emitter
{
	...
	createOutgoingStream(params) 
	{
		...
		//Create the stream 
		const outgoingStream = new OutgoingStream(streamInfo.getId());
	...
	createOutgoingStreamTrack(media, params, outgoingStream)
	{
		...
		//Set ids
		source.mid	= mediaId;
		source.rid	= encodingSources.rid	 ?? "";

		//If using rid mechanism
		if (source.rid)
		{
			//Allow ssrc-less tracks
			source.media.ssrc	= encodingSources.media;
			source.rtx.ssrc		= encodingSources.rtx;
			//source.fec.ssrc	= encodingSources.fec;
		} else {
			//Set source ssrcs
			source.media.ssrc	= encodingSources.media ?? this.lfsr.seq(31);
			//Only allow rtx/fec for video for now
			if (media == "video")
			{
				source.rtx.ssrc	= encodingSources.rtx	?? this.lfsr.seq(31);
				//source.fec.ssrc= encodingSources.fec	?? this.lfsr.seq(31);
			}
		}
