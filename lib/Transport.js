...
class Transport
{
	...
	dump(filename,options) 
	{
		...
		const incomoning = options ? Boolean(options.incomoning) : true;
		const outgoing	 = options ? Boolean(options.outgoing) : true;
		const rtcp	 = options ? Boolean(options.rtcp) : true;
		
		//Start dumping
		if (!this.transport.Dump(filename, incomoning, outgoing, rtcp))
	...
	setLocalProperties(rtp)
	{
		...
		let properties = Utils.convertRTPProperties(rtp instanceof SDPInfo ? {
