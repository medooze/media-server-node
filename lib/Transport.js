...
class Transport extends Emitter
{
		...
		/** @type {import("./Endpoint").CreateTransportOptions} */ options)
	{
		...
		this.ondatachannelmessage = (msg) => {
			console.log("ondatachannelmessage: %s", msg.toString());
		};
		
		this.messageListener = new Native.DataChannelMessageListenerShared(this);
		
		this.ondatachannelopen = (dataChannel) => {
			let allChannels = this.transport.GetDataChannels();
			console.log(allChannels);
			console.log("ondatachannelopen: %s", dataChannel.get().GetLabel());
			
			for (let c of allChannels)
			{
				console.log("channel: %s", c.get().GetLabel());
			}
			
			dataChannel.get().AddMessageListener(this.messageListener);
			dataChannel.get().Send("Hi client!");
		};
		
		this.ondatachannelclose = (dataChannel) => {
			console.log("ondatachannelclose: %s", dataChannel.get().GetLabel());
		};
