...
class Transport
{
	 ...
	 * @param {Object} params	
	 * @param {Object|boolean} params.audio	- Add audio track to the new stream
	 * @param {Number?} params.audio.media	- ssrc for the audio track
	 * @param {Object|boolean} params.video	- Add video track to the new stream
	 * @param {Number?} params.audio.media	- ssrc for the video track
	 * @param {Number?} params.video.rtx 	- ssrc for the rtx video track
	 * @param {Number?} params.video.fec	- ssrc for the fec video track
	...
	createOutgoingStream(params) 
	{
		...
		if (params.audio)
		{
			...
			//Generte new ssrc
			let ssrc = params.audio.media || this.lfsr.seq(31);
		...
		if (params.video)
		{
			...
			//Generte new ssrc
			let ssrc =  params.video.media || this.lfsr.seq(31);
			let rtx  =  params.video.rtx   || this.lfsr.seq(31);
			let fec  =  params.video.fec   || this.lfsr.seq(31);
			//Add to track
			track.addSSRC(ssrc);
			track.addSSRC(rtx);
			track.addSSRC(fec);
