...
class Transport
{
	...
	constructor(bundle, remote, local, options)
	{
		...
		//Put ice properties
		properties.SetPropertyStr("ice.localUsername"	, this.local.ice.getUfrag());
		properties.SetPropertyStr("ice.localPassword"	, this.local.ice.getPwd());
		properties.SetPropertyStr("ice.remoteUsername"	, this.remote.ice.getUfrag());
		properties.SetPropertyStr("ice.remotePassword"	, this.remote.ice.getPwd());

		//Put remote dtls properties
		properties.SetPropertyStr("dtls.setup"		, Setup.toString(remote.dtls.getSetup()));
		properties.SetPropertyStr("dtls.hash"		, remote.dtls.getHash());
		properties.SetPropertyStr("dtls.fingerprint"	, remote.dtls.getFingerprint());
		
		//Put other options
		properties.SetPropertyBool("disableSTUNKeepAlive"	, Boolean(options.disableSTUNKeepAlive));
		properties.SetPropertyStr("srtpProtectionProfiles"	, options.srtpProtectionProfiles);
		
		//Create username
		this.username = this.local.ice.ufrag + ":" + this.remote.ice.ufrag;
	...
	{
		//Set value
		this.transport.SetProbingBitrateLimit(bitrate);
	}
