 ...
 * @property {number} [upwardsLayerWaitTimeMs] Duration the Transponder will wait before switching back to a higher layer in msec, defaults to immediate == 0
 */
...
class Transport extends Emitter
{
		...
		optionalLogId, optionalLogger)
	{
		...
		// @todo logId doesnt really belong here but it is a MUCH bigger change doing this differently. We will pass it to C++ here
		if (this.optionalLogId !== undefined)
			properties.SetStringProperty("bwe.logId", this.optionalLogId);

		if (options.forceSmooth !== undefined)
			properties.SetBooleanProperty("bwe.forceSmooth", options.forceSmooth);

		if (options.enableCongestedRTX !== undefined)
			properties.SetBooleanProperty("bwe.enableCongestedRTX", options.enableCongestedRTX);

		if (options.monitorDuration !== undefined)
			properties.SetIntegerProperty("bwe.monitorDuration", options.monitorDuration);

		if (options.minRateChangeBps !== undefined)
			properties.SetIntegerProperty("bwe.minRateChangeBps", options.minRateChangeBps);

		if (options.rateChangePercentage !== undefined)
			properties.SetFloatProperty("bwe.rateChangePercentage", options.rateChangePercentage);
	...
	setBandwidthProbing(probe)
	{
		...
		if (this.optionalLogger)
		{
			this.optionalLogger.debug("setBandwidthProbing() changing probing to: %s", probe ? "enabled" : "disabled");
		}
	...
	createOutgoingStreamTrack(media, params)
	{
		...
		if (media=="video")
		{
			source.rtx.ssrc	= opts.ssrcs?.rtx ? opts.ssrcs.rtx   : this.lfsr.seq(31);
			source.fec.ssrc	= opts.ssrcs?.fec ? opts.ssrcs.fec   : this.lfsr.seq(31);
		}
		else
		{
			source.rtx.ssrc = 0;
		...
		//Create new track
		const upwardsLayerWaitTimeMs = params?.upwardsLayerWaitTimeMs || 0;
		const outgoingStreamTrack = new OutgoingStreamTrack(media, id, mediaId, this.transport.toRTPSender(), source, this.optionalLogId, this.optionalLogger, { upwardsLayerWaitTimeMs: upwardsLayerWaitTimeMs });
