...
class Transport extends Emitter
{
	...
	constructor(bundle, remote, local, options)
	{
		...
		//Put ice properties
		properties.SetProperty("ice.localUsername"	, String(this.local.ice.getUfrag()));
		properties.SetProperty("ice.localPassword"	, String(this.local.ice.getPwd()));
		properties.SetProperty("ice.remoteUsername"	, String(this.remote.ice.getUfrag()));
		properties.SetProperty("ice.remotePassword"	, String(this.remote.ice.getPwd()));

		//Put remote dtls properties
		if (options?.prefferDTLSSetupActive && remote.dtls.getSetup() == Setup.ACTPASS)
			properties.SetProperty("dtls.setup"	, String(Setup.toString(Setup.PASSIVE)));
		else
			properties.SetProperty("dtls.setup"	, String(Setup.toString(remote.dtls.getSetup())));
		properties.SetProperty("dtls.hash"		, String(remote.dtls.getHash()));
		properties.SetProperty("dtls.fingerprint"	, String(remote.dtls.getFingerprint()));
		
		//Put other options
		properties.SetProperty("disableSTUNKeepAlive"	, Boolean(options.disableSTUNKeepAlive));
		properties.SetProperty("srtpProtectionProfiles"	, String(options.srtpProtectionProfiles));

		//If disabling REMB calculus
		if (!!options.disableREMB)
			//Override it
			properties.SetProperty("remb.disabled"	, true);
		...
		this.ontargetbitrate = (targetBitrate, bandwidthEstimation, totalBitrate)  => {
			...
			* @argument {Transport} transport
			*/
			this.emitter.emit("targetbitrate",targetBitrate,bandwidthEstimation,totalBitrate,this);
		};
	...
	restartICE(remoteICE, localICE)
	{
		...
		//Put ice properties
		properties.SetProperty("ice.localUsername"	, String(localICE.getUfrag()));
		properties.SetProperty("ice.localPassword"	, String(localICE.getPwd()));
		properties.SetProperty("ice.remoteUsername"	, String(remoteICE.getUfrag()));
		properties.SetProperty("ice.remotePassword"	, String(remoteICE.getPwd()));
	...
	createOutgoingStreamTrack(media, params)
	{
		...
		//Add it to transport
		if (!this.transport.AddOutgoingSourceGroup(source[SharedPointer.Pointer]))
			//Launch exception
		...
		outgoingStreamTrack.once("stopped",()=>{
			...
			this.transport.RemoveOutgoingSourceGroup(source[SharedPointer.Pointer]);
			//Remove from tracks
	...
	createIncomingStreamTrack(media, params)
	{
		...
		//Add it to transport
		if (!this.transport.AddIncomingSourceGroup(source[SharedPointer.Pointer]))
			//Launch exception
			throw new Error("Could not add incoming source group to native transport");
		...
		incomingStreamTrack.once("stopped",()=>{
			...
			for (let id of Object.keys(sources))
				//Remove source group
				this.transport.RemoveIncomingSourceGroup(sources[id][SharedPointer.Pointer]);
			//Remove from tracks
