...
class Transport extends Emitter
{
	...
	)
	{
		...
		this.connection = SharedPointer(bundle.AddICETransport(this.username,properties,this.logger?.getName() || ""));
		//Get transport
	...
	createOutgoingStream(params) 
	{
		...
		//Create it
		const outgoingStream = new OutgoingStream(this.transport,this.lfsr,info,this.logger);
	...
	createOutgoingStreamTrack(media, params)
	{
		...
		//Create new track
		const outgoingStreamTrack = new OutgoingStreamTrack(media,id,mediaId,this.transport.toRTPSender(),source, this.logger);
