...
class IncomingStream
{
	...
	* @returns {Array<IncomingStreamTrack>}	- Array of tracks
	...
	createTrack(trackInfo)
	{
		...
		if (encodings.length)
		{
			...
			for (let i=0; i<encodings.length; ++i)
			{
				...
				for (let j=0; j<encodings[i].length; ++j)
				{
					...
					if (mid)
						source.mid = new Native.StringFacade(mid);
		...
		//Create new track
		const incomingStreamTrack = new IncomingStreamTrack(trackInfo.getMedia(),trackInfo.getId(),this.receiver,sources);

		//Add listener
		incomingStreamTrack.once("stopped",()=>{
			//REmove from map
			this.tracks.delete(trackInfo.getId());
			//For each source
		...
		//Add it to map
		this.tracks.set(trackInfo.getId(),incomingStreamTrack);
		
		/**
		* IncomingStreamTrack created
		*
		* @event IncomingStream#track
		* @type {IncomingStreamTrack}
		*/
		this.emitter && this.emitter.emit("track",incomingStreamTrack);
