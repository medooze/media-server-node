...
class IncomingStream
{
	constructor(transport,info)
	{
		...
		for (let track of info.getTracks().values())
		{
				...
				for (let i=0; i<encodings.length; ++i)
				{
					...
					for (let j=0; j<encodings[i].length; ++j)
					{
						...
						if (params && params.has("ssrc"))
						{
							...
							//Check groups
							for (let k=0; j<groups.length; ++j)
							{
								//Check if it is from us
								if (groups[k].getSSRCs()[0]===source.media.ssrc)
								{
			...
			//Create new track
			const incomingStreamTrack = new IncomingStreamTrack(track.getMedia(),track.getId(),Native.TransportToReceiver(transport),sources);
