...
class IncomingStream extends Emitter
{
	...
	constructor(
		...
		/** @type {SharedPointer.Proxy<Native.RTPReceiverShared> | null} */ receiver,
	...
	{
		const { transport, receiver } = this;
		//Check it is associated to a transport 
		if (!transport)
			//Launch exception
			throw new Error("Transport is not set");
		//Check it is associated 
		if (!receiver)
		...
		try 
		{
			...
			if (encodings.length)
			{
				...
				for (const alternative of encodings)
				{
					...
					for (const encoding of alternative)
					{
						...
						const source = SharedPointer(new Native.RTPIncomingSourceGroupShared(type,transport.GetTimeService()));
						...
						{
							//Get it
							source.media.ssrc = parseInt(ssrc);
						...
						//Add it to transport
						if (!transport.AddIncomingSourceGroup(source))
							//Launch exception
			...
			for (let id of Object.keys(sources))
				//Remove source group
				transport.RemoveIncomingSourceGroup(sources[id]);
			//Rethrow
			throw e;
		}

		//Create new track
		const incomingStreamTrack = new IncomingStreamTrack(trackInfo.getMedia(),trackInfo.getId(),trackInfo.getMediaId(),transport.GetTimeService(),receiver,sources);
