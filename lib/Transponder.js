...
class Transponder extends Emitter
{
	...
	*/
	...
	async setTargetBitrateAsync(target, options) 
	{
		...
		if (!layers.length)
		{
			console.log("bcost no layers exist so returning empty");
		...
		for (let layer of layers)
		{
			...
			)
			{
					...
					|| temporalLayerId != this.temporalLayerId)
				{
					...
					console.log(`bcost Choosing a higher quality layer! layerBitrate:${layerBitrate} target:${target} current:${current} maxSpatialLayerId:${this.maxSpatialLayerId} spatialLayerId:${layer.spatialLayerId} maxTemporalLayerId:${this.maxTemporalLayerId} temporalLayerId:${layer.temporalLayerId}`);
				}
		...
		if (current<=0)
		{
			...
			if (!options || !options["strict"])
			{
				console.log(`bcost no current layer but not strict so choosing min layer`);
				...
				console.log(`bcost no current layer but strict so muting`);
				//Mute it
				this.mute(true);
				//Not sending anything
				return Object.assign(new Number(0),{
		...
		if (!this.loggedId
			...
			|| temporalLayerId != this.temporalLayerId)
		{
			console.log(`bcost switching layer to ${spatialLayerId} with encoding ${encodingId} smooth:${smooth} (newSilmulcastIdx:${newSilmulcastIdx} prevSimulcastIdx:${prevSimulcastIdx}) and temporal ${temporalLayerId}`);
			this.loggedId = true;
		}
