	...
	*/
	...
	setTargetBitrate(target, options, logstr) 
	{
		...
		if (!layers.length)
		{
			...
			{
				console.debug(`bcost [${this.id}]-Transponder::setTargetBitrate() No layers exist yet so returning default encoding ${fallback.id}${logstr}`);
		...
		if (current<=0)
		{
			...
			{
				console.debug(`bcost [${this.id}]-Transponder::setTargetBitrate() Unable to locate layer matching bitrate requirements, so using min: ${encodingIdMin}${logstr}`);
				...
				console.warn(`bcost [${this.id}]-Transponder::setTargetBitrate() Unable to locate layer matching bitrate requirements and strict mode so muting${logstr}`);
	...
	async setTargetBitrateAsync(target, options, logstr) 
	{
		...
		switch (options && options.traversal)
		{
			...
			default:
				...
				//else
				//{
				//	console.log("bcost no max width/height set so NO order is set on the layers!");
				//}
		}
		...
		if (!layers.length)
		{
			...
			if (options?.useDefaultEncoding && fallback)
			{
				console.debug(`bcost [${this.id}]-Transponder::setTargetBitrateAsync() No layers exist yet so using fallback ${fallback.id}${logstr}`);
				//Try to select it
				this.selectEncoding(fallback.id, options?.smooth);
			}
		...
		for (let layer of layers)
		{
			...
			)
			{
				...
				if (encodingId != this.encodingId)
				{
					...
					console.debug(`bcost [${this.id}]-Transponder::setTargetBitrateAsync() Auto switching to layer ${encodingId} from ${this.encodingId} layerBitrate:${layerBitrate} target:${target} spatialLayerId:${layer.spatialLayerId} temporalLayerId:${layer.temporalLayerId}${logstr}`);
				}
		...
		if (current<=0)
		{
			...
			if (!options || !options["strict"])
			{
				console.warn(`bcost [${this.id}]-Transponder::setTargetBitrateAsync() No layer matched requirements so choosing min layer ${encodingIdMin}${logstr}`);
				...
				console.warn(`bcost [${this.id}]-Transponder::setTargetBitrateAsync() No layer mateched requirements and we are in strict mode so muting this transponder${logstr}`);
		...
		//if (encodingId != this.encodingId)
		//{
		//	console.debug(`bcost [${this.id}]-Transponder::setTargetBitrateAsync() Auto switching to layer ${encodingId} with spatialLayerId:${spatialLayerId}, smooth:${smooth} (newSilmulcastIdx:${newSilmulcastIdx} prevSimulcastIdx:${prevSimulcastIdx}) and temporal ${temporalLayerId}${logstr}`);
		//}
	...
	selectEncoding(encodingId,smooth) 
	{
		...
		console.debug(`bcost [${this.id}]-Transponder::selectEncoding() Switching to encoding "${encodingId}"`);
