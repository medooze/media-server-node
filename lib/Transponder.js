...
class Transponder extends Emitter
{
	...
	*/
	...
	setTargetBitrate(target, options, logstr) 
	{
		...
		//For Paddy Power - force smooth switching
		const smooth = true || !options || options.smooth || newSilmulcastIdx >= prevSimulcastIdx;
		//Select enccoding
	...
	async setTargetBitrateAsync(target, options, logstr) 
	{
		...
		for (let layer of layers)
		{
			...
			if (this.nextEncodingSwitchTime <= Date.now())
			{
				...
				this.logger.log(`bcost DEBUGGING switching to layer ${layer.encodingId} and changing next encoding to: ${this.nextEncoding}`)
				isReady = true;
			}
			...
			{
				//this.logger.log(`bcost skipping layer ${layer.encodingId} as it doesnt match expected next layer: ${this.nextEncoding}`)
				continue;
			}
			*/
	...
	selectEncoding(encodingId,smooth) 
	{
		...
		this.logger.debug(`-Transponder::selectEncoding() Switching to encoding "${encodingId}" from "${this.encodingId}"`);
