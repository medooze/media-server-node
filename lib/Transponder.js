...
class Transponder
{
	...
	setIncomingTrack(track, layers)
	{
		...
		if (this.track)
		{
			...
			if (this.track.getMedia()=="video")
			{
				//Set defaults
				const curated = Object.assign({
					encodingId		: this.track.encodings.values().next().value.id,
				},layers);
	...
	*/
	...
	select(layers)
	{
		//Set defaults
		const curated = Object.assign({
			encodingId		: this.getSelectedtEncoding(),
		...
		//Start listening to the encoding
		this.selectEncoding(curated.encodingId);
