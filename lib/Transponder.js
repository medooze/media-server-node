...
class Transponder extends Emitter
{
	...
	constructor(
		...
		/** @type {(import("./Logger")|undefined)} */ logger)
	{
		//Init emitter
		super();
	...
	*/
	...
	{
		// @todo I dont think this function is called anywhere except the tests, will remove as another PR
		...
		if (!layers.length)
		{
			...
			{
				this.logger?.debug(`-Transponder::setTargetBitrate() No layers exist yet so returning default encoding ${fallback.id}`);
		...
		for (let layer of layers)
		{
			...
			)
			{
				...
				// If the encoding we are selecting is different, then log why we are selecting it to allow some debugging of rapid layer switching
				if (encodingId != this.encodingId)
					this.logger?.debug(`-Transponder::setTargetBitrate() Auto switching to layer ${encodingId} from ${this.encodingId} layerBitrate:${layerBitrate} target:${target} spatialLayerId:${layer.spatialLayerId} temporalLayerId:${layer.temporalLayerId} layerWidth:${(layer.width || layer.targetWidth)} maxWidth:${this.maxWidth} layerHeight:${(layer.height || layer.targetHeight)} maxHeight:${this.maxHeight}`);
		...
		if (current<=0)
		{
			...
			{
				this.logger?.warn(`-Transponder::setTargetBitrate() Unable to locate layer suitable for target bitrate ${target}, so choosing min layer ${encodingIdMin} with bitrate: ${layers[layerMinIndex].bitrate} and target ${layers[layerMinIndex].targetBitrate}`);
	...
	async setTargetBitrateAsync(target, options) 
	{
		...
		if (!layers.length)
		{
			...
			if (options?.useDefaultEncoding && fallback)
			{
				this.logger?.debug(`-Transponder::setTargetBitrateAsync() No layers exist yet so returning default encoding ${fallback.id}`);
				//Try to select it
				this.selectEncoding(fallback.id, options?.smooth);
			}
		...
		if (current<=0)
		{
			...
			if (!options || !options["strict"])
			{
				...
				{
					this.logger?.warn(`-Transponder::setTargetBitrateAsync() Unable to locate layer suitable for target bitrate ${target}, so choosing min layer ${encodingIdMin} with bitrate: ${layers[layerMinIndex].bitrate} and target ${layers[layerMinIndex].targetBitrate}`);
				}
