...
class Transponder extends Emitter
{
	...
	*/
	...
	{
		console.log(`bcost ${TS()} DONT THINK THIS METHOD IS USED WHY DOES IT EXIST`);
		...
		if (!layers.length)
		{
			...
			{
				console.debug(`bcost ${TS()} [${this.id}]-Transponder::setTargetBitrate() No layers exist yet so returning default encoding ${fallback.id}${logstr}`);
		...
		if (current<=0)
		{
			...
			{
				console.debug(`bcost ${TS()} [${this.id}]-Transponder::setTargetBitrate() Unable to locate layer matching bitrate requirements, so using min: ${encodingIdMin}${logstr}`);
				...
				console.warn(`bcost ${TS()} [${this.id}]-Transponder::setTargetBitrate() Unable to locate layer matching bitrate requirements and strict mode so muting${logstr}`);
	...
	async setTargetBitrateAsync(target, options, logstr) 
	{
		...
		switch (options && options.traversal)
		{
			...
			case "zig-zag-temporal-spatial":
				...
				break;
			default:
				// @todo Ordering not set unless max width/height set. Are they?
				...
				// @todo Seems we dont have width/height and so are not ordering the layers.
				// They happen to be ordered already I guess
				//else
				//{
				//	console.log("bcost ${TS()} no max width/height set so NO order is set on the layers!");
				//}
		}
		...
		if (!layers.length)
		{
			// @todo useDefaultEncoding is set to !fixed so if the later is auto then use default
			...
			if (options?.useDefaultEncoding && fallback)
			{
				console.debug(`bcost ${TS()} [${this.id}]-Transponder::setTargetBitrateAsync() No layers exist yet so using fallback ${fallback.id}${logstr}`);
				//Try to select it
				this.selectEncoding(fallback.id, options?.smooth);
			}
		...
		for (let layer of layers)
		{
			// @todo I assume layer.bitrate is what was last measured, otherwise target is what things are telling us they are targeting with this layer
			//Use the max of the actual and signaled target bitrate
			const layerBitrate = layer.targetBitrate ? Math.max(layer.bitrate, layer.targetBitrate) : layer.bitrate;


			// If layer fits in available space AND
			// its bitrate is higher than previous layers we looked at
			// and id is in allowable range 
			// @todo what are the max ids used to prevent?
			//
			// @todo I think "current" is fairly useless in here now... Not sure why we need layerBitrate>current as soon as we find a valid match we use it
			// I assume the ordering was added later?
			...
			if (this.nextEncodingSwitchTime <= Date.now())
			{
				...
				{
					this.nextEncoding = '540p';
				}
				else
				{
					this.nextEncoding = '720p';
				}
				console.log(`bcost ${TS()} DEBUGGING switching to layer ${layer.encodingId} and changing next encoding to: ${this.nextEncoding}`)
				isReady = true;
			}
			...
			{
				//console.log(`bcost ${TS()} skipping layer ${layer.encodingId} as it doesnt match expected next layer: ${this.nextEncoding}`)
				continue;
			}
			*/
			...
			)
			{
				...
				if (encodingId != this.encodingId)
				{
					...
					console.debug(`bcost ${TS()} [${this.id}]-Transponder::setTargetBitrateAsync() Auto switching to layer ${encodingId} from ${this.encodingId} layerBitrate:${layerBitrate} target:${target} spatialLayerId:${layer.spatialLayerId} temporalLayerId:${layer.temporalLayerId}${logstr}`);
				}
		...
		if (current<=0)
		{
			...
			if (!options || !options["strict"])
			{
				console.warn(`bcost ${TS()} [${this.id}]-Transponder::setTargetBitrateAsync() No layer matched requirements so choosing min layer ${encodingIdMin}${logstr}`);
				...
				console.warn(`bcost ${TS()} [${this.id}]-Transponder::setTargetBitrateAsync() No layer mateched requirements and we are in strict mode so muting this transponder${logstr}`);
		...
		// @todo This is where we are doing the layer switching
		//if (encodingId != this.encodingId)
		//{
		//	console.debug(`bcost ${TS()} [${this.id}]-Transponder::setTargetBitrateAsync() Auto switching to layer ${encodingId} with spatialLayerId:${spatialLayerId}, smooth:${smooth} (newSilmulcastIdx:${newSilmulcastIdx} prevSimulcastIdx:${prevSimulcastIdx}) and temporal ${temporalLayerId}${logstr}`);
		//}
	...
	selectEncoding(encodingId,smooth) 
	{
		...
		console.debug(`bcost ${TS()} [${this.id}]-Transponder::selectEncoding() Switching to encoding "${encodingId}" from "${this.encoding}"`);
