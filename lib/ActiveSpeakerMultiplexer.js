	...
	constructor(timeService,streamOrTracks)
	{
		...
		this.onactivespeakerchanged = (speakerId,multiplexeId) => {
			...
			*/
			...
			const outgoingStreamTrack	= this.multiplexed.get(multiplexeId)
		...
		this.onOutgoingTrackStopped = (outgoingTrack) => {
			//Get transponder
			const transponder = outgoingTrack.getTransponder();
			//If it still has it
			if (transponder)
				//Remove from multiplexer
				this.multiplexer.RemoveRTPStreamTransponder(transponder.transponder);
		};
