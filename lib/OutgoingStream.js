...
/** @typedef {import("./Transport").CreateStreamTrackOptions & { media: SemanticSDP.TrackType }} CreateTrackOptions */
...
class OutgoingStream extends Emitter
{
	 ...
	 * @param {CreateTrackOptions | TrackInfo | SemanticSDP.TrackType} params Params plain object, StreamInfo object or media type
	...
	createTrack(params)
	{
		...
		//Get media type
		//@ts-expect-error
		const media = /** @type {SemanticSDP.TrackType} */ (params.constructor.name === "TrackInfo" ? params.getMedia() : params && params.media ? params.media : params);
		...
		//Create new track
		const outgoingStreamTrack = new OutgoingStreamTrack(media, trackId, mediaId, this.transport.toRTPSender(), source, this.logger);
