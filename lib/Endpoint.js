 ...
 * @property {string[]} [ips] Override announced IPs for this transport (if not passed, defaults to endpoint IPs)
 */
...
class Endpoint extends Emitter
{
	...
	createTransport(remoteInfo, localInfo, options)
	{
		...
		const local = parsePeerInfo(localInfo || {
			...
			candidates	: Endpoint.generateLocalCandidates(options?.ips ?? this.ips, this.getLocalPort()),
		});
	...
	getLocalCandidates() 
	{
		...
		return Endpoint.generateLocalCandidates(this.ips, this.getLocalPort());
	}
	...
	static generateLocalCandidates(
		/** @type {string[]} */ ips,
		/** @type {number} */ port)
	{
		return ips.map((ip, i) => {
			//Calculate priority in descending order
			const priority = Math.pow(2,24)*126 + Math.pow(2,8)*(65535-i) + 255;
			//Add new RTP UPD local candidate
			return new CandidateInfo("1", 1, "UDP", priority, ip, port, "host");
		});
	}
}
