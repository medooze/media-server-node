...
class Endpoint 
{
	...
	constructor(ip)
	{
		...
		for (let i=0; i<this.ips.length; i++) {
			let priority = Math.pow(2,24) + (65535-i)*Math.pow(2,8) + 255;
			this.candidates.push(new CandidateInfo("1", 1, "UDP", priority, ip, this.bundle.GetLocalPort(), "host"))
		}
	...
	mirrorIncomingStream(incomingStream)
	{
		...
		if (!mirroredStream)
		{
			...
			for (const incomingStreamTrack of incomingStream.getTracks())
				//Add mirror track
				mirroredStream.addTrack(this.mirrorIncomingStreamTrack(incomingStreamTrack));
	...
	{
		//Get mirrored track
		let mirroredTrack = this.mirrored.tracks.get(incomingStreamTrack);
		
		//If not mirrored yet
		if (!mirroredTrack)
		{
			//Create new stream
			mirroredTrack = new IncomingStreamTrackMirrored(incomingStreamTrack,this.bundle.GetTimeService());
			//Listen for stop event
