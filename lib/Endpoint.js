 ...
 * @property {String} [logId] TODO This doesnt belong here but it is a MUCH larger change to pass loggers and logId to other objects. Waiting for logging update
 * @property {boolean} [forceSmooth]
 * @property {boolean} [enableCongestedRTX]
 * @property {number} [bweMonitorDuration]
 * @property {number} [bweMinRateChangeBps]
 * @property {number} [bweRateChangePercentage]
 */
...
class Endpoint extends Emitter
{
	 ...
	 */
	createTransport(remoteInfo, localInfo, options, optionalLogId, optionalLogger)
	{
		...
		const transport = new Transport(this.bundle, remote, local, Object.assign({
			...
			}, options), optionalLogId, optionalLogger
		);
	...
	mirrorIncomingStream(incomingStream)
	{
		...
		if (!mirroredStream)
		{
			...
			mirroredStream.on("trackremoved", (incomingStream, incomingStreamTrack) => {
				...
				this.mirrored.mirrors.delete(incomingStreamTrack);
			});
