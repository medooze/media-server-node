...
class Endpoint 
{
	...
	createTransport(remoteInfo, localInfo, options)
	{
		...
		if (remoteInfo instanceof SDPInfo)
			//Convert
			remoteInfo = {
				dtls		: remoteInfo.getDTLSInfo(),
				ice		: remoteInfo.getICEInfo(),
		...
		const remote = {
			dtls		: remoteInfo.dtls instanceof DTLSInfo ? remoteInfo.dtls.clone() : DTLSInfo.expand(remoteInfo.dtls),
			ice		: remoteInfo.ice  instanceof ICEInfo  ? remoteInfo.ice.clone()  : ICEInfo.expand(remoteInfo.ice),
			candidates	: []
		};
		...
		for (let i=0;remoteInfo.candidates && i<remoteInfo.candidates.length; ++i)
			//Clone
			remote.candidates.push(remoteInfo.candidates[i] instanceof CandidateInfo ? remoteInfo.candidates[i].clone() : CandidateInfo.expand(remoteInfo.candidates[i]));
		
		//Get local Info
		//TODO: support also plain objects
		const local = {
			ice	: localInfo && localInfo.ice		? localInfo.ice.clone()	 : ICEInfo.generate(true),
			dtls	: localInfo && localInfo.dtls		? localInfo.dtls.clone() : new DTLSInfo(Setup.reverse(remoteInfo.dtls.getSetup()),"sha-256",this.fingerprint),
			//TODO: should clone also
			candidates : localInfo && localInfo.candidates	? localInfo.candidates	 : [this.candidate]
		};
